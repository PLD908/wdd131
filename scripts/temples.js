...
function displayTemples(filteredTemples) {
  ...
  if (filteredTemples.length === 0) {
    templeContainer.innerHTML = "<p>No temples match this criteria.</p>";
    return;
  }
...
navLinks.forEach(link => {
  link.addEventListener("click", (event) => {
    ...
    const filterFunction = filterTemples[filterType.toLowerCase()];
    const filteredTemples = filterFunction();
    displayTemples(filteredTemples);
    
    const mobileView = document.querySelector(".mobile-view");
    const desktop = document.querySelector(".desktop");
    if (window.getComputedStyle(mobileView).display === "block") {
      mobileView.style.display = "none";
      desktop.style.display = "flex";
    }
  });
